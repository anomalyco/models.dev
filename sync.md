...
- Resolving a relay to a lab model peels three kinds of affix to a fixed point, because they stack: routing prefixes that name the upstream compute or an AIHubMix namespace (`alicloud-`, `coding-`, `ahm-`), routing suffixes that select a mode rather than a different model (`-free`, `-nothink`, `-search`), and dated release tags AIHubMix pins onto the ID (`-preview-05-06`, `-2026-01-23`, `-260215`). The date patterns are anchored and validate real month and day ranges, so `llama2-70b-4096` keeps its context size. The unstripped ID is tried first, so a lab that genuinely carries a date in its name (`cohere/command-a-03-2025`) still wins over a stripped candidate.
