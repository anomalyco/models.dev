...
- The `verbose=true` endpoint has been observed returning incomplete/throttled samples for some models — a context length pinned to a `8_000` default, and `supported_features`/`architecture.modality` missing capabilities an existing entry already confirmed. To guard against that:
  - `limit.context` is only accepted from the API when it's at least as large as what's already known from the existing entry or canonical metadata, and never below the input limit.
  - `tool_call` and `structured_output` only ever gain from an API sample, never lose a capability an existing entry already had confirmed.
  - `modalities` are unioned with the existing entry's modalities rather than replaced, so an under-reported sample can't drop a previously-confirmed modality (and `attachment`, derived from `modalities.input`, follows).
  - A brand-new model (no existing entry, no canonical `models/` match) with a `context_length` of exactly `8_000` — the empirically-confirmed throttled default — is skipped entirely rather than created with an unverifiable context, since there's nothing to cross-check it against; it's reported via `skippedNotice` for a future re-sync once account-tier/API-key throttling is resolved.
- `reasoning_options` is preserved from existing TOMLs as-is; the models-discovery API only exposes a boolean `reasoning` capability, not the toggle/effort mechanism. `provider.toml` documents the underlying chat completions endpoint generically accepting `reasoning_effort`, but that's wire-level acceptance, not evidence a given model's backend uses it — reasoning models already catalogued here diverge by family (GLM/gpt-oss/DeepSeek use `effort`, Qwen3.5 uses `toggle`, the Nemotron siblings use neither). A new reasoning model should only get an `effort`/`toggle` option hand-curated when a same-family sibling on this provider already demonstrates that mechanism; otherwise leave `[]`.
- New remote models are created automatically. A small `NEBIUS_CANONICAL_PROVIDER_PREFIXES` map (scoped to orgs Nebius currently hosts) resolves known orgs (`meta-llama`, `Qwen`, `google`, `openai`, `nvidia`, `zai-org`, `moonshotai`, `MiniMaxAI`, `deepseek-ai`) to their `models/` namespace, reusing OpenRouter's shared `createCanonicalBaseModelResolver`/`modelMetadataExists`/`canonicalLimit` helpers rather than duplicating the file-scan/cache logic. `factorBaseModel` (shared with OpenRouter) writes a `base_model` reference when a matching canonical entry exists — this is how new models get an authoritative output token limit despite the Models API not exposing one. New models with no canonical `models/` match (unmapped orgs, or a checkpoint not yet in `models/`) are still created as full entries, with `limit.output` falling back to the served context length, unless skipped per the throttled-context guard above.
- `limit.input`, `limit.output` (for already-factored/full existing entries), `knowledge`, `family`, `status`, `interleaved`, and cache pricing ratios are preserved from existing TOMLs since the API does not expose them.
