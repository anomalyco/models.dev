...
- `preserveDescriptions` is off: the runner must not re-inject a pre-factor authored description when the translator leaves it unset, or a full-inline → factored transition would recreate a lab-identical override. Created files get a leading header from `translateModel` naming the reasoning wire path (a `toggle` is `reasoning_effort = "none"` versus the graded levels on the same field).
