...
- `reasoning_options` is preserved from existing TOMLs as-is; the models-discovery API only exposes a boolean `reasoning` capability, not the toggle/effort mechanism `provider.toml` documents for the underlying chat completions endpoint, so a new reasoning model added here needs its `effort`/`toggle` option hand-curated (matching sibling entries) rather than defaulting to `[]`.
