...
- The API is authoritative for display names, temperature metadata, and the `thinking` flag when present. Token limits normally come from the API, with a small allowlist of model-card overrides for documented cases where the generic endpoint lags or reports endpoint plumbing limits.
