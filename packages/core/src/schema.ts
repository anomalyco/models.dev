...
export const ModelParameters = z
  .object({
    total: z
      .number()
      .int("Total parameters must be a whole number")
      .min(1, "Total parameters must be positive")
      .safe(),
    active: z
      .number()
      .int("Active parameters must be a whole number")
  ...
  .refine(
    (data) =>
      ...
      data.architecture === "moe" ||
      data.architecture === "hybrid",
    {
      message:
        "Active parameters are only allowed when architecture is \"moe\" or \"hybrid\" (hybrid models with MoE routing)",
      path: ["active"],
    },
  );
