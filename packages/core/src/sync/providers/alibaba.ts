...
const AlibabaPriceRange = z.object({
  range_name: z.string(),
  prices: z.array(AlibabaPrice),
}).passthrough();

const AlibabaModelInfo = z.object({
...
async function fetchModelsPage(apiEndpoint: string, apiKey: string, pageNo: number) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Alibaba model catalog request failed: ${response.status} ${response.statusText}: ${body}`);
  }

  const page = AlibabaCatalogResponse.parse(await response.json());
  if (!page.success) {
    throw new Error(`Alibaba model catalog request failed: ${page.code ?? "unknown"}: ${page.message ?? "unknown error"}`);
  }
  return page;
}
...
function costFromPrices(prices: z.infer<typeof AlibabaPrice>[], existing: Cost | undefined): Cost | undefined {
  const input = price(
    prices,
    "input_token",
    "text_input_token",
  ...
  const imageOutput = price(prices, "image_number", "image_standard", "image_thinking");
  const duration = price(prices, "content_duration");
  const tts = price(prices, "cosy_tts_number");

  if (input === undefined && output === undefined && imageOutput === undefined && duration === undefined && tts === undefined) {
    return undefined;
  }
...
function cost(model: AlibabaModel, existing: ExistingModel) {
  ...
  const tiers = ranges.slice(1).map((range): CostTier => ({
...
function modalities(model: AlibabaModel, existing: ExistingModel) {
  ...
  const output = normalizedModalities(model.inference_metadata.response_modality);
...
function requireExisting<T>(model: AlibabaModel, field: string, value: T | undefined): T {
  if (value === undefined) {
    throw new Error(`Alibaba model ${model.model} has incomplete local TOML metadata required for sync: ${field}`);
  }
  return value;
}

export function buildAlibabaModel(model: AlibabaModel, existing: ExistingModel): SyncedModel {
