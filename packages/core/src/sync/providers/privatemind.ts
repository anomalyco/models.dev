...
// Aliases for ids the normalizer cannot resolve on its own: a canonical file
// named for a version code (Mistral Medium 3.5 lives at mistral-medium-2604),
// and deployments that serve one checkpoint behind a suffix naming the serving
// window rather than the model (Kimi K3 at 128K and 500K).
const BASE_MODEL_ALIASES: Record<string, string> = {
  "mistral-medium-3-5-128b-nvfp4": "mistral/mistral-medium-2604",
  "kimi-k3-chat": "moonshotai/kimi-k3",
  "kimi-k3-long": "moonshotai/kimi-k3",
};
...
export const privatemind = {
  ...
  skippedNotice(ids) {
    ...
    return [
      `${ids.length} PrivateMind models were not created because no price could be resolved: /v1/models quotes cost to API-key callers only, and these have no authored \`[cost]\` to fall back on. Set \`PRIVATEMIND_API_KEY\` to create them.`,
