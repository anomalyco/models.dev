...
// Mirrors schema.ts ReasoningEffortValue (not exported). Strict on purpose:
// a level this catalog doesn't know fails the sync loudly instead of
// writing an invalid reasoning option.
const EffortLevel = z.enum(["none", "minimal", "low", "medium", "high", "xhigh", "max", "default"]);
...
const Capabilities = z
  .object({
    ...
    // Accepted values of a graded per-model dial (e.g. kimi-k3's
    // low | high | max). Absent on binary-toggle hybrids.
...
export const privatemind = {
  ...
  translateModel(model, context) {
    ...
    const synced: SyncedFullModel = {
      ...
      // low | high | max, always thinking, no off). Hybrids carry the bare
      ...
      reasoning_options: reasoning
        ? effortLevels.length > 0
          ? [{ type: "effort", values: effortLevels }]
          : effortToggle
            ? [{ type: "toggle" }]
            : []
        : undefined,
    ...
    if (baseModel === undefined) return { id: model.id, model: synced };
    const { release_date: _releaseDate, last_updated: _lastUpdated, ...overrides } = synced;
    return { id: model.id, model: factorBaseModel(baseModel, overrides, synced.limit) };
  },
