...
import { formatToml, preserveReasoningOptions, syncProvider, type SyncedModel, type SyncProvider } from "../src/sync/index.js";
...
test("defaults new reasoning models to empty reasoning options", () => {
  const model = { reasoning: true } as SyncedModel;

  expect(preserveReasoningOptions(model, undefined)).toEqual({
    reasoning: true,
    reasoning_options: [],
  } as unknown as SyncedModel);
});
