...
const renderUsageBasedCost = (cost?: number) => {
  return cost === undefined ? "-" : `${cost.toFixed(2)}x`;
};
            ...
            Object.entries(provider.models)
              ...
              .map(([modelId, model]) => (
                <tr key={`${providerId}-${modelId}`}>
                  ...
                  <td>{providerId == "github-copilot" ? renderUsageBasedCost(model.cost?.input) : renderCost(model.cost?.input)}</td>
                  <td>{providerId == "github-copilot" ? renderUsageBasedCost(model.cost?.output) : renderCost(model.cost?.output)}</td>
