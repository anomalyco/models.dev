...
- Apply **`AGENTS.md` → Reasoning options** and `.opencode/skills/audit-reasoning-options/SKILL.md` exactly. Classify each changed model as OpenAI-compatible gateway vs native (from `npm` / `api` / model `[provider]`), then:
  ...
  - **`none` vs `toggle`:** flag `type = "toggle"` **together with** `effort` values that already include `none` (double-counted off) as a violation. Do **not** flag `toggle` + graded effort (`low`/`medium`/`high`/… without `none`) when off is a separate boolean/enabled control — that shape is valid (e.g. DeepSeek V4). Every `toggle` requires a **leading top-of-file comment** describing the wire on/off path; missing comment is a violation.
