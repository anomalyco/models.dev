...
active = 37_000_000_000            # MoE only: per-token activated params
...
- **Raw units, always.** `total = 671_000_000_000`, not `671B`. Underscores are valid TOML and encouraged.
- **`active` only for MoE** (`architecture = "moe"`); it must not exceed `total`.
